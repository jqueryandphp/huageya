...
function _stringToStream() {
  const data = _interopRequireDefault(require("string-to-stream"));

  _stringToStream = function _stringToStream() {
    return data;
  };
...
const server = config => {
  const root = config.root,
        ...
        _config$stream = config.stream,
        stream = _config$stream === void 0 ? false : _config$stream,
  ...
  const polyfillHost = (polyfill === null || polyfill === void 0 ? void 0 : polyfill.host) || 'http://localhost';
    ...
    var _ref = _asyncToGenerator(function* (ctx, renderOpts = {}) {
      ...
      if (stream) {
        return _objectSpread({
          ssrStream: (0, _stringToStream().default)(ssrHtml)
        }, result);
